...
I'll be adding new results for different salts as I generate them, but there are **many** to explore. If you've created a new S_ice surface/lookup function for a salt that isn't already included and are willing to contribute it to the catalog, push your 'saltname' folder to your forked repository and create a pull request to add it to the main branch of the master.

### Steps
Step 0 - only do once) If you don't already have a fork of the master repository, create your own fork of the main branch ('Fork' button at top right of [main branch page](https://github.com/jbuffo/SOFTsurf)). Add the master repository as an upstream so you can quickly make sure your fork is up to date:
...
Step 1) Make sure your fork (or local branch) is up to date with the master repository:

```bash
git pull upstream main
```

Step 2) Generate or add in your new 'saltname' folder (which contains the variables, lookup functions, and surface for that salt) in the 'Salts' folder

Step 3) Push your updates to your fork:

```bash
git push origin your-fork      # Where 'your-fork' is the name of your forked repository, default is likely 'main'
```

Step 4) Create a pull request to merge your updates with the master. Go to your forked repository and hit the 'Contribute' button to open a pull request. Fill out the information with a description of your additions.
