function SOFTsurf = makeSurface(salinity,slope,C_e,beta,g,k_s,saltname,path)
    ...
    for i=1:num_salinities
        ...
        % initial guess values for coefficients based on paper fit (is this the best option?)
        initguess=[7.864,-2576,-5.148,-2067,-869.2,-10.9,27.2];
        % fit curve and create values along the fit line
        f=fit(sort_hold_mat(:,2),sort_hold_mat(:,3),ft,'StartPoint',initguess);
